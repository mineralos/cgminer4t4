...
const int max_expiry = 600000000;//600;
...
int opt_voltage = 175;
// for G19
int opt_voltage1 = 175;
int opt_voltage2 = 175;
int opt_voltage3 = 175;
int opt_voltage4 = 175;
int opt_voltage5 = 175;
int opt_voltage6 = 175;
...
struct stratum_share *stratum_shares = NULL;
...
bool pool_tset(struct pool *pool, bool *var)
{
    bool ret;
...
static struct opt_table opt_config_table[] = {
    ...
    // for G19
    OPT_WITH_ARG("--A1Vol1",
             set_int_0_to_255, opt_show_intval, &opt_voltage1,
             "set voltage (1 ~ 175)"),
    OPT_WITH_ARG("--A1Vol2",
             set_int_0_to_255, opt_show_intval, &opt_voltage2,
             "set voltage (1 ~ 175)"),
    OPT_WITH_ARG("--A1Vol3",
             set_int_0_to_255, opt_show_intval, &opt_voltage3,
             "set voltage (1 ~ 175)"),
    OPT_WITH_ARG("--A1Vol4",
             set_int_0_to_255, opt_show_intval, &opt_voltage4,
             "set voltage (1 ~ 175)"),
    OPT_WITH_ARG("--A1Vol5",
             set_int_0_to_255, opt_show_intval, &opt_voltage5,
             "set voltage (1 ~ 175)"),
    OPT_WITH_ARG("--A1Vol6",
             set_int_0_to_255, opt_show_intval, &opt_voltage6,
             "set voltage (1 ~ 175)"),
    OPT_WITH_ARG("--A1Vol7",
             set_int_0_to_255, opt_show_intval, &opt_voltage7,
             "set voltage (1 ~ 175)"),
    OPT_WITH_ARG("--A1Vol8",
             set_int_0_to_255, opt_show_intval, &opt_voltage8,
             "set voltage (1 ~ 175)"),

    OPT_WITH_ARG("--A1Fanspd",
             set_int_0_to_100, opt_show_intval, &opt_fanspeed,
             "set fan speed (0 ~ 100)"),
    OPT_WITH_ARG("--A1Fanmode",
             set_int_0_to_5, opt_show_intval, &opt_fanmode,
             "set fan control (manual ~ auto)"),
             
    OPT_WITH_ARG("--A1Heattime",
             set_int_0_to_10, opt_show_intval, &opt_heattime,
             "set fan control (manual ~ auto)"),
...
double cgpu_runtime(struct cgpu_info *cgpu)
{
    ...
    if (cgpu->dev_start_tv.tv_sec == 0){
        ...
        applog(LOG_DEBUG, "secs %f, start %d:%d, end %d:%d \n", 
            total_secs, total_tv_start.tv_sec, total_tv_start.tv_usec, total_tv_end.tv_sec, total_tv_end.tv_usec);
    }
    else {
        cgtime(&now);
        dev_runtime = tdiff(&now, &(cgpu->dev_start_tv));
        applog(LOG_DEBUG, "now %d:%d, tv %d:%d \n", 
            now.tv_sec, now.tv_usec, cgpu->dev_start_tv.tv_sec, cgpu->dev_start_tv.tv_usec);
    }
...
void show_hash(struct work *work, char *hashshow)
{
...
void pool_died(struct pool *pool)
{
    if (!pool_tset(pool, &pool->idle)) {
        cgtime(&pool->tv_idle);
        if (pool == current_pool()) {
            applog(LOG_WARNING, "Pool %d not responding!", pool->pool_no);
            //applog(LOG_WARNING, "Pool %d %s not responding!", pool->pool_no, pool->rpc_url);
            switch_pools(NULL);
        } else {
            applog(LOG_INFO, "Pool %d failed to return work", pool->pool_no);
...
void switch_pools(struct pool *selected)
{
    ...
    if (pool != last_pool && pool_strategy != POOL_LOADBALANCE && pool_strategy != POOL_BALANCE) {
        applog(LOG_WARNING, "Switching to pool%d", pool->pool_no);
...
void _discard_work(struct work **workptr, const char *file, const char *func, const int line)
{
    ...
    if (unlikely(!work)) {
        applog(LOG_ERR, "Discard work called with null work from %s %s:%d", file, func, line);
        return;
    }
...
static void *restart_thread(void __maybe_unused *arg)
{
    ...
    applog(LOG_DEBUG, "enter restart_thread");
...
bool test_work_current(struct work *work)
{
    struct pool *pool = work->pool;
...
bool hash_push(struct work *work)
{
    //applog(LOG_DEBUG, "hash_push()");
...
void _stage_work(struct work *work)
{
...
bool cnx_needed(struct pool *pool)
{
    struct pool *cp;
...
void wait_lpcurrent(struct pool *pool);
...
bool supports_resume(struct pool *pool)
{
    bool ret;
...
static void *stratum_rthread(void *userdata)
{
    ...
    while (42) {
        ...
        if (!sock_full(pool) && (sel_ret = select(pool->sock + 1, &rd, NULL, NULL, &timeout)) < 1) {
            ...
            applog(LOG_DEBUG, "recv_line in stratum_rthread");
            s = recv_line(pool);
        }
        if (!s) {
...
{
    applog(LOG_INFO, "Testing pool%d", pool->pool_no);
...
static bool pool_active(struct pool *pool, bool pinging)
{
    ...
    if (pool->has_gbt)
        applog(LOG_DEBUG, "Retrieving block template from pool%d", pool->pool_no);
    else
        applog(LOG_INFO, "Testing pool%d", pool->pool_no);

    //add by wangpeng fot getwork
    if(opt_getwork)
        return pool_active_getwork(pool, pinging);
...
static void pool_resus(struct pool *pool)
{
    pool->seq_getfails = 0;
    if (pool_strategy == POOL_FAILOVER && pool->prio < cp_prio())
        applog(LOG_WARNING, "Pool%d alive, testing stability", pool->pool_no);
    else
        applog(LOG_INFO, "Pool%d alive", pool->pool_no);
}
...
static struct work *hash_pop(bool blocking)
{
    ...
    //applog(LOG_DEBUG, "enter hash_pop()");
...
out_unlock:
    ...
    //applog(LOG_DEBUG, "out hash_pop() %p", work);
...
struct work *get_work(struct thr_info *thr, const int thr_id)
{
    ...
    applog(LOG_DEBUG, "Got work from get queue for thread %d", thr_id);
    ...
    //applog(LOG_DEBUG, "Popping work from get queue to get work success");
    return work;
}
...
{
    //applog(LOG_DEBUG, "enter fill_queue()");
    ...
    //applog(LOG_DEBUG, "out fill_queue()");
}
...
void hash_queued_work(struct thr_info *mythr)
{
    ...
    //applog(LOG_DEBUG, "enter hash_queued_work()");

    while (likely(!cgpu->shutdown)) {
        struct timeval diff;
        int64_t hashes;
        
        if(a8_miner_cnt == thr_id)
        {
            fill_queue(mythr, cgpu, drv, thr_id);
            mutex_lock(&a8_miner_lock);
            a8_miner_cnt = (a8_miner_cnt + 1) % total_devices;
            mutex_unlock(&a8_miner_lock);
        }
        else
        {
            usleep(5000);
...
void *scan_thread(void *userdata)
{
    struct thr_info *mythr = userdata;
    const int thr_id = mythr->id;
    struct cgpu_info *cgpu = mythr->cgpu;
    ...
    snprintf(threadname, sizeof(threadname), "%d/ScanMiner", thr_id);
    RenameThread(threadname);
    applog(LOG_DEBUG, "creat scan thread %s", threadname);

    //applog(LOG_DEBUG, "Waiting on sem in scan thread");
    //cgsem_wait(&mythr->sem);
    sleep(5);
    
    scan_nonce_hash(mythr);
    ...
    while (42) {
        ...
        for (i = 0; i < total_pools; i++) {
            ...
            if (!pool->idle && pool_strategy == POOL_FAILOVER && pool->prio < cp_prio() &&
                ...
                applog(LOG_WARNING, "Pool %d stable for >%d seconds", pool->pool_no, opt_pool_fallback);
                switch_pools(NULL);
            }
        }
    ...
    if (pool_active(pool, false)) {
        ...
        if (!pools_active) {
            ...
            if (pool->pool_no != 0)
                first_pool = true;
            pools_active = true;
            //applog(LOG_NOTICE, "set pools_active true");
        }
        cg_wunlock(&control_lock);

        if (unlikely(first_pool))
            applog(LOG_NOTICE, "Switching to pool%d - first alive pool", pool->pool_no);
...
# ifdef __linux
    ...
    mutex_init(&a8_miner_lock);
    mutex_init(&rpc2_job_lock);
    ...
    for (i = 0; i < total_pools; i++) {
        ...
        if (!pool->rpc_userpass) {
            if (!pool->rpc_pass)
                pool->rpc_pass = strdup("");
            if (!pool->rpc_user)
                early_quit(1, "No login credentials supplied for pool %u", i);
            siz = strlen(pool->rpc_user) + strlen(pool->rpc_pass) + 2;
            pool->rpc_userpass = cgmalloc(siz);
            snprintf(pool->rpc_userpass, siz, "%s:%s", pool->rpc_user, pool->rpc_pass);
        }
    ...
    while (!pools_active) {
        if (!pool_msg) {
            ...
            for (i = 0; i < total_pools; i++) {
                ...
                applog(LOG_DEBUG, "Pool: %d  URL: %s  User: %s  Password: %s", i, pool->rpc_url, pool->rpc_user, pool->rpc_pass);
            }
            pool_msg = true;
            if (use_curses)
