...
bool tq_push(struct thread_q *tq, void *data)
{
    ...
    //applog(LOG_DEBUG, "enter tq_push()");
    ...
    //applog(LOG_DEBUG, "out tq_push() rc:%d", rc);
...
out:
    ...
    //applog(LOG_DEBUG, "out tq_pop() rval:%p", rval);
...
bool auth_stratum(struct pool *pool)
{
    ...
    while (42) {
        applog(LOG_DEBUG, "recv_line in auth_stratum");
        sret = recv_line(pool);
        if (!sret)
            return ret;
...
bool setup_stratum_socket(struct pool *pool)
{
    ...
    if (getaddrinfo(sockaddr_url, sockaddr_port, &hints, &servinfo) != 0) {
        if (!pool->probed) {
            applog(LOG_WARNING, "Failed to resolve (?wrong URL)");
            //applog(LOG_WARNING, "Failed to resolve (?wrong URL) %s:%s", sockaddr_url, sockaddr_port);
            pool->probed = true;
        } else {
            applog(LOG_INFO, "Failed to getaddrinfo for pool %d", pool->pool_no);
            //applog(LOG_INFO, "Failed to getaddrinfo for %s:%s", sockaddr_url, sockaddr_port);
        }
        return false;
    }
...
retry:
    ...
    if (p == NULL) {
        ...
        //applog(LOG_INFO, "Failed to connect to stratum on %s:%s", sockaddr_url, sockaddr_port);
        freeaddrinfo(servinfo);
        return false;
    }
    freeaddrinfo(servinfo);
    ...
    if (pool->rpc_proxy) {
        switch (pool->rpc_proxytype) {
            ...
            case PROXY_SOCKS4A:
                ...
                break;
            default:
                applog(LOG_WARNING, "Unsupported proxy type for %s:%s", pool->sockaddr_proxy_url, pool->sockaddr_proxy_port);
                return false;
                break;
        }
    }
...
resend:
    ...
    applog(LOG_DEBUG, "recv_line in initiate_stratum");
    sret = recv_line(pool);
    if (!sret)
        goto out;
