...
void Encrypt(unsigned char *InText, unsigned char *ExpKey, unsigned char *OutText)
{
    ...
    for(i=0;i<Nb;i++)
    {
     if (i%4==0)
    		printf("\n");
      printf("%#x ",State[4*i+0]);
      printf("%#x ",State[4*i+1]);
      printf("%#x ",State[4*i+2]);
      printf("%#x ",State[4*i+3]);
    }
    printf("\n"); 
    */
...
bool AES_Encrypt(unsigned char *Key, unsigned char *InText, int len, unsigned char *OutText)
{
    ...
    memcpy(str1, InText, len);
    //printf("str1=%s \n", str1);   
    
    ExpandKey(Key, ExpKey);

    memset(str2, 0x00, sizeof(str2));
    for(i = 0; i < tmp_len; i = i + 16) {
        Encrypt((unsigned char *)(str1 + i), ExpKey, (unsigned char *)(str2 + i));
    }
    //printf("str2=%s \n", str2);
    
    memcpy(OutText, str2, tmp_len);
...
bool AES_Decrypt(unsigned char *Key, unsigned char *InText, int len, unsigned char *OutText)
{
    ...
    memset(str2, 0x00, sizeof(str2));
    for(i = 0; i < tmp_len; i = i + 16) {
        Decrypt((unsigned char *)(str1 + i), ExpKey, (unsigned char *)(str2 + i));
    }
    //printf("str2=%s \n", str2);
...
void main()
{
    //unsigned char in[16]={0x00,0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f};
    //unsigned char in[16]={0x0e,0x0f, 0x0e, 0x0f, 0x0e, 0x0f, 0x0e, 0x0f, 0x0e, 0x0f, 0x0e, 0x0f, 0x0e, 0x0f, 0x0e, 0x0f};
    //unsigned char in[16]={0x1e,0x2f, 0xd3, 0xa1, 0x2c, 0xe3, 0x1a, 0x2a, 0xe3, 0xe1, 0x2c, 0x3c, 0x1f, 0xf2, 0xd3, 0x1c};
    unsigned char in[16]={0x8e,0x4f, 0x80, 0xa7, 0x2c, 0x43, 0x10, 0x20, 0x73, 0x4f, 0x2c, 0x3c, 0x40, 0xfa, 0xd0, 0x4c};

    //unsigned char in[16]={0x11,0x22, 0x33, 0x11, 0x22, 0x33, 0x11, 0x22, 0x33, 0x11, 0x22, 0x33, 0x11, 0x22, 0x33, 0x11};
    //unsigned char in[16]={0x32,0x43, 0xf6, 0xad, 0x88, 0x5a, 0x30, 0x8d, 0x31, 0x31, 0x98, 0xa2, 0xe0, 0x37, 0x7, 0x34};
